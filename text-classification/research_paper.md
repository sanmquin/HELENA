...
Large language models are increasingly deployed in domains where ground-truth labels are absent, contested, or inherently subjective. Evaluating model behavior in such settings demands metrics that go beyond exact-match accuracy. This paper studies a description-based text classification task in which LLM-generated category descriptions serve as the sole interface between training data and a classifier. The central empirical result is that categories whose descriptions better preserve the topology of the underlying data are classified more accurately (Pearson r = 0.56, p = 0.004). Three complementary results clarify why: central categories are harder to quantify (r = 0.44 between distinctiveness and F1, p = 0.03), close categories are more easily confused (r = -0.24 between distance and confusion, p < 0.0001), and high-volume categories may be harder to classify (r = -0.32 between dispersion and recall, p = 0.13). Together, these findings suggest that distance-preservation metrics can identify which descriptions are geometrically faithful, which descriptions distort the domain, and where classification difficulty should be expected before labels are available.
